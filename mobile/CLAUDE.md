...
autofill is **non-blocking** — its loading state is scoped to the Bookmark section (which dims +
shows an "Autofilling…" indicator) so the list picker stays live, and the picker hydrates instantly from a
