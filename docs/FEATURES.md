...
- **Save rich bookmarks.** Each bookmark holds a name, description, photos, notes, a rating (0–5
  stars in half-star steps), a location, a "visited" checkmark, and multiple links — not just a bare URL.
...
`ListPageHeader` (identity block + a **⋮ actions menu** via
`ListToolbar`, on the list-name row) and a rounded-pill **List | Members | Polls** segmented tab bar
(`ListTabs`: **List** = `/lists/[id]`, **Members** = `/lists/[id]?tab=members`, **Polls** = `/lists/[id]?tab=polls`, rendered inline so the
...
list** → `lists/edit`, **Duplicate list** →
...
target), extracted photos (`images[]`), free-form notes, a location, a 0–5 rating (half-star steps), a visited flag,
and user tags.
...
**Description.** Rate a bookmark 0–5 stars **in half-star steps** (tap the left half of a star for
½, the right half for a full star; tapping the current value again clears to 0), toggle a "visited"
flag, and keep multiline notes. Both the rating and the visited flag are editable **inline on the
detail view** as well as in the add/edit form — the two surfaces use the same controls. Rating is
stored as a `Float` and clamped/snapped to half-steps in core (`Math.round(x*2)/2`).
...
`RatingInput` + `VisitedField` (matched pair). Rating input/display share `EditableStars` (two
half-width hit targets per star) and a fractional-fill overlay. Notes textarea.
**Mobile.** Shared `RatingStars` (half-star tap-to-rate, optimistic `bookmarks.setRating`, compact
`size="sm"` read-only variant on the list card) + `VisitedPill` (optimistic
`bookmarks.toggleVisited`) on the detail screen; the same two controls in the add/edit form. Notes field.
**Differences.** None.
...
friend request. Non-owners can leave a shared list. The **member roster is viewable by every
member** as a read-only **Members tab** on the list view (`sharing.members` is VIEWER-guarded);
only the owner sees the invite/role/remove controls.
**Web.** `MembersPanel` (takes `canManage` — owner gets the controls, others read-only) rendered by
the **Members tab** (`?tab=members`) + `/requests` page (linked from a home button) +
`/invite/[token]`; `sharing.*` procedures (`invite`, `incomingRequests`, `approveRequest`,
`rejectRequest`, …); `assertRole` enforced on every mutation server-side.
**Mobile.** `src/components/list-members.tsx` rendered by the **Members tab** on
`src/app/lists/[id].tsx` (owner controls gated on `isOwner`) + a pushed `src/app/requests.tsx`
screen (reached from a **List requests** button above the home search), same `sharing.*` procedures.
